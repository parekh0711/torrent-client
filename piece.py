...
def create_have_request(socket,bitfield,data):
    ...
    for index,piece in enumerate(data):
        if piece=='0' and bitfield[index]=='1':
            ...
            # print(buf)
            socket.send(buf)
            resp=socket.recv(20000)
            parse_piece_request(resp,index)
            data= data[:index]+'1'+ data[index+1:]
            # return buf
    print("data = ",data)

def parse_piece_request(resp,off):
    length = unpack_from("!i",resp)
    id = unpack_from("!b",resp,4)
    piece = unpack_from("!i",resp,5)
