    ...
    while True:
        ...
        print(
            "Downloading from {} peers at {} kbps.".format(
                len(connected_peers), round(current_download_speed / 125, 2)
            )
        )
        sleep(2)
        if 0 not in recieved_data:
    ...
    for tracker in trackers:
        ...
        try:
            if tracker.scheme == "udp":
                ...
                try:
                    connection_id = udp_parse_connection_response(
                ...
                # print(t)
                try:
                    response = requests.get(t + "?" + urlencode(params), timeout=2)
                ...
                try:
                    ...
                    for index in range(len(peers)):
                        ...
                        if len(messages) >= 1:
                            ...
                            # print(flag)
                        if (
                            not flag or not peer_bitfield
                        ):  # at this point, we have bitfield
                            # print(flag,bitfield)
                            continue
                        t = Thread(
                            target=active_peer,
